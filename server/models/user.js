...
class User {
  ...
  insertUser(username, password, full_name, dni, avatar, res, req) {
    ...
    conn.query(sql1, [username], (err0, results0) => {
      if (err0) {
        res.status(400).send({
          OK: false,
          error: "No s'ha pogut fer la sentencia de comprovacio",
        });
      } else {
        if ((results0.length == 0)) {
          let sql =
            "INSERT INTO users (username, password, full_name,avatar) " +
            "VALUES (?,?,?,?)";
          conn.query(
            sql,
            [username, password, full_name, avatar],
            (err, results) => {
              if (err) {
                console.log(err);
                res.status(401).send({
          ...
          res.status(400).send({
            OK: false,
            error: "No puc inserir l'usuari perquè el username està duplicat",
          });
        }
      }
    });
